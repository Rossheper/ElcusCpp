...
DWORD TmkOpen()
{
	...
	int nTMKb = 0;
	int badver = 0;
	char tmkName[tmkNameSize];
	char cc = 0x5c;

	for (iTMK = 0; iTMK <= MAX_TMK_NUMBER; ++iTMK)
	{
		_VTMK4usbFlag[iTMK] = 0;
		sprintf_s(tmkName,tmkNameSize, "%c%c.%cTMK1553BDevice%d",cc,cc,cc,iTMK);
		...
		if(loaded)
		{
			...
			for(iTMK = 0; iTMK <= MAX_USBTA_NUM; ++iTMK)
			{
				if(nMRT+1+nTMKb > MAX_USBTA_NUM)
					break;
				sprintf_s (tmkName,tmkNameSize,"%c%c.%cEZUSB-%d",cc,cc,cc,iTMK);
...
int tmkconfig(int tmkNumber)
{
	...
	{
		sprintf_s (tmkName,tmkNameSize,"%c%c.%cEZUSB-%d",0x5c,0x5c,0x5c,_VTMK4tmkDrvnum[tmkNumber]);
	...
	sprintf_s (tmkName,tmkNameSize,"%c%c.%cTMK1553BDevice%d",0x5c,0x5c,0x5c,tmkNumber);
	_hVTMK4VxD[tmkNumber] = CreateFileA(tmkName,
		GENERIC_READ | GENERIC_WRITE,
		0,
		NULL,
...
DWORD mrtconfig(int mrtNumber)
{
	...
	{	
		sprintf_s (tmkName,tmkNameSize,"%c%c.%cTMK1553BDevice%d",0x5c,0x5c,0x5c,mrtNumber);
		...
		if (_awVTMK4OutBuf[0] != 0 && _awVTMK4OutBuf[1] != 0)
		{
			...
			for (iRT = RT0; iRT < RTN; ++iRT)
			{
				...
				sprintf_s (tmkName,tmkNameSize,"%c%c.%cTMK1553BDevice%d",0x5c,0x5c,0x5c,iRT);
