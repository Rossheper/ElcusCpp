...
Manchester::Manchester() {
	...
	_pz = new unsigned short *[2];
	for (int i = 0; i < 2; i++) {
		_pz[i] = new unsigned short[32];
	}
	for (int i = 0; i < 32; i++) {
		_sendingMass[i] = 0;
	}
...
//Обрабатываем прерывание на получение данных
int Manchester::WaitInt(unsigned short wCtrlCode) {
	switch (WaitForSingleObject(this->_currHandle, 1000))
	{
	case WAIT_OBJECT_0:
...
bool Manchester::sendData(short oldAddr, short addrOy, short K, short subAddr, short ncd, unsigned short * massToSend) {
	...
	if (K == 0) {
		...
		do {
			if (bcstartx(this->_wBase, sendingControl(K) | CX_STOP | CX_BUS_A | CX_NOSIG) != 0)
				return false;
